...
from ._ext import (
    WaveLUT_cforward, WaveLUT_cbackward
)

r"""4D transformation.
We follow the [ACM MM FASTLLVE](https://github.com/Wenhao-Li-777/FastLLVE) implementation. 
See Section III for more details.
"""

class WaveLUTTransformFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        WaveLUT_cforward(video, lut, output)
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        WaveLUT_cbackward(grad_output, video, lut, grad_video, grad_lut)

        return grad_video, grad_lut


def WaveLUT_transform(
    video: torch.Tensor,
    lut: torch.Tensor) -> torch.Tensor:
    ...
    """
    return WaveLUTTransformFunction.apply(video, lut)